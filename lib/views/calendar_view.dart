...
/// Calendar view: Daily (main), Weekly (overview), Monthly (date picker).
...
class _CalendarViewState extends State<CalendarView> {
  ...
  CalendarViewMode _viewMode = CalendarViewMode.daily;
  ...
  Widget build(BuildContext context) {
    ...
    final scheduledVM = Provider.of<ScheduledTaskViewModel>(context, listen: false);
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              child: _viewMode == CalendarViewMode.daily
                  ? _buildDailyView(context, taskVM, scheduledVM)
                  : _buildWeeklyView(context, taskVM, scheduledVM),
            ),
          ],
        ),
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => _showAddTaskDialog(context, taskVM, scheduledVM),
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(
                  _viewMode == CalendarViewMode.daily ? 'Daily' : 'This week',
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              return Wrap(
                ...
                children: [
                  _viewChip('Daily', CalendarViewMode.daily),
                  _viewChip('Weekly', CalendarViewMode.weekly),
                  _buildGenerateScheduleButton(scheduledVM, compact: compact),
                ],
              );
            },
          ),
  ...
  Widget _buildGenerateScheduleButton(
    ...
    final taskVM = Provider.of<TaskViewModel>(context, listen: false);
    final label = compact ? 'Generate' : 'Generate Schedule';
    return GestureDetector(
      onTap: () => _showGenerateScheduleOptions(taskVM, scheduledVM),
      child: Container(
  ...
  Future<void> _onGenerateSchedule(
    ...
    final existing = await scheduledVM.fetchScheduledTasksForDate(_selectedDate);
    final scheduledTaskIds = existing.map((st) => st.taskId).toSet();
    final occupiedBlocks = existing
        .map((st) => ExistingBlock(startMinutes: st.startTime, endMinutes: st.endTime))
        .toList();
    final candidates = tasks.where((t) => !scheduledTaskIds.contains(t.id)).toList();
    ...
    final toSchedule =
        candidates.where((t) => selectedTaskIds.contains(t.id)).toList();
  ...
  Future<void> _onGenerateScheduleInCustomRange(
    ...
    final unscheduledTasks =
        tasks.where((t) => !scheduledTaskIds.contains(t.id)).toList();
                              ...
                              children: [
                                ...
                                _selectionActionChip(
                                  ...
                                  onTap: () => setSheetState(() {
                                    selectedTaskIds
                                      ..clear()
                                      ..addAll(unscheduledTasks.map((t) => t.id));
                                  }),
                                ),
                      ...
                      Expanded(
                        child: ListView(
                          ...
                          children: [
                            _taskSectionCard(
                              title: 'Overdue tasks',
                              subtitle: unscheduledTasks.where((t) => t.overdue).isEmpty
                                  ? 'None'
                              ...
                              child: () {
                                final overdue =
                                    unscheduledTasks.where((t) => t.overdue).toList();
                                if (overdue.isEmpty) {
                                  return Padding(
                            ...
                            const SizedBox(height: 12),
                            _taskSectionCard(
                              title: 'Tasks for ${_formatHeaderDate(_selectedDate)}',
                              subtitle: unscheduledTasks.where((t) => !t.overdue).isEmpty
                                  ? 'None'
                                  : '${unscheduledTasks.where((t) => !t.overdue).length} task${unscheduledTasks.where((t) => !t.overdue).length == 1 ? '' : 's'}',
    ...
    final userPrefsVM =
        Provider.of<UserPreferencesViewModel>(context, listen: false);
    final prefs = await userPrefsVM.fetchPreferences();
  ...
  Widget _viewChip(String label, CalendarViewMode mode) {
    final selected = _viewMode == mode;
    return GestureDetector(
      onTap: () {
        setState(() => _viewMode = mode);
        // Scroll to 6am when switching to daily view
        ...
        return StreamBuilder<List<TaskModel>>(
          ...
          builder: (context, taskSnapshot) {
            ...
            final scheduledForUi = taskSnapshot.hasData
                ? scheduled.where((st) => taskMap.containsKey(st.taskId)).toList()
                : scheduled;
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  _buildTimeSlots(context, scheduledVM, taskVM, scheduledForUi, taskMap),
                ],
              ),
            );
          },
  ...
  Widget _buildTimeSlots(
    ...
    for (final block in scheduled) {
      ...
      double calculatedHeight = (block.endTime - block.startTime) / 60 * _slotHeight;
      ...
      final blockTitle = task?.title ??
          (block.taskName.trim().isNotEmpty ? block.taskName : 'Scheduled block');
  ...
  void _autoBackfillScheduledForDay({
    ...
    for (final task in allTasks) {
      ...
      final hasForSelectedDate = existing.any((st) =>
  ...
  Widget _buildScheduledTaskCard(
    ...
    return GestureDetector(
      onTap: () {
        ...
        _showScheduledTaskDetails(context, st, title, scheduledVM, taskVM, task);
      },
      ...
      onVerticalDragEnd: (details) async {
        ...
        try {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Time updated to ${_minutesToTime(newStart)} – ${_minutesToTime(newEnd)}'),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            if (cardHeight > 44) _buildResizeHandle(),
            Expanded(
              child: isShortTask
                  ? Center(
                      child: FittedBox(
                        ...
                        child: Row(
                          ...
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.flag_outlined, size: 12, color: statusColor),
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          StreamBuilder<List<CategoryModel>>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              if (categories.isEmpty) {
                                return const Text('No categories. Add one in Tasks.');
                              }
                          ...
                          const SizedBox(height: 12),
                          Text('Urgency', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.grey[800])),
                          Row(
                            children: [
                              Expanded(
                                child: SliderTheme(
                                  ...
                                  child: Slider(
                                    ...
                                    max: 5,
                                    divisions: 4,
                                    onChanged: (v) => setDialogState(() => urgency = v.toInt()),
                                  ),
                                ),
                              ),
                              Container(
                                ...
                                child: Center(child: Text('$urgency', style: const TextStyle(fontWeight: FontWeight.bold))),
                              ),
                            ],
                          ),
                          Text('Importance', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.grey[800])),
                          Row(
                            children: [
                              Expanded(
                                child: SliderTheme(
                              ...
                              Container(
                                ...
                                child: Center(child: Text('$importance', style: const TextStyle(fontWeight: FontWeight.bold))),
                              ),
                            ],
                          ),
                        ],
                        ...
                        Expanded(
                          child: ElevatedButton(
                            onPressed: () async {
                              ...
                              Navigator.pop(ctx);
                              if (taskId != null) {
                                final startMinutes = scheduleTime.hour * 60 + scheduleTime.minute;
                                try {
                                ...
                                if (ctx.mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Added: $title (Tasks & Calendar)'),
                                      backgroundColor: Colors.black,
                                      behavior: SnackBarBehavior.floating,
                                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                                    ),
                            ...
                            child: const Text('Add Task', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                          ),
                        ),
                      ],
                    ),
                              ...
                              child: Column(
                                children: [
                                  ...overdue.asMap().entries.map((entry) {
                                    ...
                                    final endMinutes = startMinutes + task.duration;
                                    return Column(
                                      children: [
                                        _taskPickRow(
                                          title: task.title,
                            ...
                            Expanded(
                              child: ElevatedButton(
                                onPressed: () {
                                  ...
                                  Navigator.pop(ctx, Set<String>.from(selectedTaskIds));
                                },
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: Colors.black,
                                ),
                                ...
                                if (remindersEnabledState) ...[
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      ...
                                      for (final minutes in (selectedOffsetsState
                                              .where(
                                                (m) =>
                                                    !_presetReminderOffsets
                                                        .contains(m),
                                              )
                                              .toList()
                                            ..sort()))
                                ...
                                Material(
                                  ...
                                  child: InkWell(
                                    onTap: () async {
                                      ...
                                      setState(() => _removedFromCalendarKeys
                                          .add(removedKey));
                                      try {
                                        ...
                                        if (context.mounted) {
                                          ScaffoldMessenger.of(context)
                                              .showSnackBar(
                                            SnackBar(
                                              content: Text('Removed "$title" from calendar'),
                                              behavior: SnackBarBehavior.floating,
                                              backgroundColor: Colors.black,
  ...
  Widget _buildWeeklyView(
    ...
    return FutureBuilder<Map<DateTime, List<ScheduledTaskModel>>>(
      future: scheduledVM.fetchScheduledTasksForRange(days.first, days.last),
      builder: (context, snapshot) {
        final map = snapshot.data ?? {};
        return StreamBuilder<List<TaskModel>>(
          stream: taskVM.tasksStream,
          builder: (context, taskSnapshot) {
            final taskMap = {for (var t in taskSnapshot.data ?? []) t.id: t};
            final tasksKnown = taskSnapshot.hasData;
            return ListView(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              children: days.map((day) {
                final rawList = map[DateTime(day.year, day.month, day.day)] ?? [];
                final list = tasksKnown
                    ? rawList.where((st) => taskMap.containsKey(st.taskId)).toList()
                    : rawList;
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (list.isEmpty)
                            ...
                            ...list.take(3).map((st) {
                              final task = taskMap[st.taskId];
                              final title = task?.title ??
                                  (st.taskName.trim().isNotEmpty ? st.taskName : 'Scheduled block');
                              return Padding(
                                padding: const EdgeInsets.only(top: 6),
                                child: Row(
                                  children: [
                                    Text(
                                      '${_minutesToTime(st.startTime)}',
                                      style: TextStyle(
                                        fontSize: 12,
                                        color: Colors.grey[600],
                                      ),
...
enum CalendarViewMode { daily, weekly }
